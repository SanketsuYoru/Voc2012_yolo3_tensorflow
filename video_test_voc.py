...
def yolo_eval(box_xy, box_wh, box_confidence, box_class_probs,image_shape, 
            max_boxes=15, score_threshold=0.2,iou_threshold=0.5):
    """
...
def pridect(origin_img,images,model):
    ...
    #使用非最大值抑制
    scores, boxes, classes = yolo_non_max_suppression(scores, boxes, classes, 15, 0.5)
    yolo_utils.draw_boxes(origin_img, scores, boxes, classes, class_names, colors)
...
def camera_test(yolo_model,path):
    # 0是代表摄像头编号，只有一个的话默认为0
    capture =cv.VideoCapture(path)
    capture.set(cv.CAP_PROP_FRAME_WIDTH, 1920) 
    capture.set(cv.CAP_PROP_FRAME_HEIGHT,1080)
    fourcc = cv.VideoWriter_fourcc(*'XVID')
    out = cv.VideoWriter('output.MP4',-1,20, (1920,1080))
    while (True):
...
def camera_test_v2(path):
    ...
    while (True):
        ...
        # img = cv.resize(frame, (int(608), int(608)), interpolation=cv.INTER_NEAREST)
        # #cv2 frame to image
        # img = Image.fromarray(np.uint8(img))
        # x = image.img_to_array(img)/255.0
        # x = np.expand_dims(x, axis=0)
        ...
        # images=Image.fromarray(np.uint8(frame))
        #pridect
        res=yolo.detect_image(Image.fromarray(np.uint8(frame)))
        #frame = cv.resize(frame, (96),(96))
        display=np.array(res)
        cv.imshow("Yolo", display)
